...
if (isset($_GET['export_excel']) && $_GET['export_excel'] == '1') {
    try {
        ...
        // Build subquery for late entry aggregation per student
        ...
        if (!empty($start_date) && !empty($end_date)) {
            ...
            $safe_start = $con->real_escape_string($start_date);
            $sub_sql .= " AND e.log_date >= '$safe_start'";
        } elseif (!empty($end_date)) {
            $safe_end = $con->real_escape_string($end_date);
            $sub_sql .= " AND e.log_date <= '$safe_end'";
        }
        ...
        // Apply minimum late count filter
        if (!empty($min_late_count) && is_numeric($min_late_count)) {
            $safe_min = (int)$min_late_count;
            $src .= " AND agg.late_count >= $safe_min";
        }

        // Order by late count descending (highest first), then by last late date/time
        $src .= " ORDER BY agg.late_count DESC, agg.last_late_date ASC, agg.last_late_time ASC";
        ...
        // Column headings (keep all columns for export)
...
<div class="main-content">
    ...
    <!-- Student List with Filters (grouped by student, with late count) -->
    <div class="student-list-container">
        <div class="section-header" style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <form method="get" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                ...
                <!-- Minimum Late Count filter -->
                <input type="number" name="min_late_count" placeholder="Min late count" value="<?php echo isset($_GET['min_late_count']) ? htmlspecialchars($_GET['min_late_count']) : ''; ?>" min="0" style="width: 120px;">
        ...
        ?>
                <!-- Add style for highlighting rows with late_count >= 3 -->
                <style>
                    .highlight-row {
                ...
                <table class="student-table">
                    ...
                    <tbody id="studentTableBody">
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $highlight_class = ($row['late_count'] >= 3) ? 'class="highlight-row"' : '';
                        ?>
        ...
        <?php
                ...
                if (!empty($min_late_count) && is_numeric($min_late_count)) {
                    $filters_applied[] = "min late count " . (int)$min_late_count;
                }
