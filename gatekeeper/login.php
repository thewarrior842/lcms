...
<head>
    ...
    <meta name="Description" content="Gatekeeper Login System">
            ...
            <div class="col-md-6 col-lg-5">
                <div class="login-card">
                    <div class="login-header text-center">
                        <h1 class="mb-3"><i class="fas fa-lock"></i> Gatekeeper Portal</h1>
                        <p class="mb-0">Secure Access for Gatekeepers</p>
                    </div>
                    ...
                    <div class="login-form">
                            ...
                            try {
                                ...
                                if ($rs->num_rows > 0) {
                                    ...
                                    if (password_verify($pwd, $row['pwd'])) {
                                        if ($row['role'] == "gatekeeper") {
                                            $gsrc = "SELECT g.*, u.* FROM gatekeeper g INNER JOIN user u ON g.email_id=u.email_id WHERE g.email_id='$email_id' AND g.status='1'";
                                            $grs = $con->query($gsrc);
                                            if ($grs->num_rows > 0) {
                                                $grow = $grs->fetch_assoc();
                                                unset($grow['pwd']);
                                                $log_date = date("Y-m-d");
                                                $log_time = date("H:i");
                                                $upd = "UPDATE user SET log_date='$log_date',log_time='$log_time' WHERE uid=" . $grow['uid'];
                                                $con->query($upd);
                                                $_SESSION['g_info'] = $grow;
                        ?>
                                                <script>
                        ...
                        <?php
                                                ...
                                                $error = "Your account already removed or suspended";
                                            }
                                        }
                                    } else {
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            toggleButton.addEventListener('click', function() {
                ...
                // Toggle the eye icon
                if (type === 'text') {
                    // Password is visible: show eye-slash to indicate visibility
