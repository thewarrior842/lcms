...
if (isset($_GET['export_excel']) && $_GET['export_excel'] == '1') {
    try {
        ...
        $filename = 'late_entries_summary_' . $filter_date . '.csv';
...
<!-- Main Content (full header and structure assumed to be present in header.php) -->
<!-- The following content should be placed after header include -->

<!-- Student List with Filters (grouped by student, default to today's date) -->
<div class="student-list-container">
    <div class="section-header" style="display: flex; justify-content: space-between; align-items: center;">
        <h3>Search Student of Late Entry</h3>
        <form method="get" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
            <!-- Date input defaults to today if not set -->
            <br><input type="date" name="filter_date" value="<?php echo !empty($_GET['filter_date']) ? htmlspecialchars($_GET['filter_date']) : date('Y-m-d'); ?>">
    ...
    <?php
            ...
            $filters_applied[] = "date " . htmlspecialchars($filter_date); // Always show date since default is today
            if (!empty($search_name)) {
